...
for N in {1..2}
do
 gfsh start server –bind-address=${GEODE_SERVERIP} --locators=${GEODE_IP}[${GEODE_LOCATOR_PORT}] --name=server$N  --server-port=0 --mcast-port=0 --cache-xml-file=scripts/server-cache.xml --include-system-classpath
done
